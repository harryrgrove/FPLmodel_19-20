...
def simulateFixtures(playerName, fixtureCount, SD=5):
    ...
    for matchID, matchInfo in dict(fixtures).items():
        points.append(simulatePts(position, playerName, 'home' * (matchInfo['loc'] == 'h') + 'away' * (matchInfo['loc'] == 'a'), matchInfo['opponent'], SD))
    return points
...
gw = 25
player = "Rashford"
print(getPlayer(player, 'name', 'name'), sum(simulateFixtures(player, 100, SD)[0:gw - 18]), str(simulateFixtures(player, 100, SD)[0:gw - 18]))
player = 'Y Ings'
print(getPlayer(player, 'name', 'name'), sum(simulateFixtures(player, 100, SD)[0:gw - 18]), str(simulateFixtures(player, 100, SD)[0:gw - 18]))
print('\n' * 2)

n = 3
#print(simulateTeamFixtures({'GKP': ['pope'], 'DEF': ['n kelly', 'soyunc', 'lundst'], 'MID': ['n son', 'dele alli', 'pulisic', 'grealish', 'sterling'], 'FOR': ['vardy', 'ny ings', 'raul Jimenez']}, n, SD))
...
print('\n')
print('Mean among top 20 assets: ' + str(np.mean([i[1] for i in sorted(scores, key=lambda i: i[1], reverse=True)[:30]]))[:4])
