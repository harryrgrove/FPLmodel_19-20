...
def rawDataUpdate():
    ...
    async def main():
        async with aiohttp.ClientSession() as session:
            ...
            results = await understat.get_league_results("epl", 2019)  # Adds the Understat info from every match from the 2018/19 and 2019/20 season to the dataset
            teams = set()
            for matchInfo in results:
                teams.update([matchInfo['h']['title'], matchInfo['a']['title']])  # Iterates through matches to add teams to teams set

            matchDB, playerDB, teamDB, fixtureDB = OrderedDict(), {team: {} for team in teams}, {team: {} for team in teams}, {team: OrderedDict() for team in teams}  # Sets up starting conditions for the creation of database dicts

            for i, team in enumerate(teams):
                fixtures = await understat.get_team_fixtures(team, 2019)
                for fixture in fixtures:
                    print(fixture)
                    fixtureDB[team][int(fixture['id'])] = {'loc': fixture['side'], 'opponent': fixture['a' * (fixture['side'] == 'h') + 'h' * (fixture['side'] == 'a')]['title']}
                print('\n' * 20 + str(i + 1) + '/20 fixtures updated')

            totalProgress, countProgress, startTime = len(results), 0, time()  # Initialises timing variables to give time estimate

            for matchInfo in results:
                homeTeam, awayTeam, matchID = matchInfo['h']['title'], matchInfo['a']['title'], int(matchInfo['id'])  # Assigns variables for the home team, away team and ID of each fixture
                matchDB[matchID] = {'homeTeam': homeTeam, 'awayTeam': awayTeam, 'hxG': float(matchInfo['xG']['h']), 'axG': float(matchInfo['xG']['a']), 'hxA': 0, 'axA': 0, 'homeScore': int(matchInfo['goals']['h']), 'awayScore': int(matchInfo['goals']['a']), 'date': matchInfo['datetime'][0:10]}  # Adds fixture data to the matchDB dict

                if homeTeam not in teamDB:
                    teamDB[homeTeam] = {}
                if awayTeam not in teamDB:
                    teamDB[awayTeam] = {}  # Adds a team to teamDB if not already added
                teamDB[homeTeam][matchID] = {'location': 'home', 'opponent': awayTeam, 'xG': float(matchInfo['xG']['h']), 'xC': float(matchInfo['xG']['a'])}
                teamDB[awayTeam][matchID] = {'location': 'away', 'opponent': homeTeam, 'xG': float(matchInfo['xG']['a']), 'xC': float(matchInfo['xG']['h'])}  # Adds team data of the fixture to teamDB. This is used in formulating the Strength Value Matrix in teamFDR.py.

                countProgress += 1
                timeProgress = time() - startTime
                print('\n' * 20 + 'Estimated Time Remaining: ' + str((timeProgress / countProgress) * (totalProgress - countProgress))[0:str((timeProgress / countProgress) * (totalProgress - countProgress)).find('.')] + ' seconds')  # Provides elementary estimate for time remaining in data update

                for match in [await understat.get_match_players(matchID)]:  # Iterates through all PL matches in the dataset
                    for team, playerDict in match.items():  # Iterates through both teams in the given match
                        if team == 'h':  # For home team
                            for playerID, playerAttributes in playerDict.items():  # Iterates through every player in the given team
                                ...
                                playerDB[homeTeam][playerAttributes['player_id']][matchID] = {'mins': int(playerAttributes['time']), 'xG': float(playerAttributes['xG']), 'xA': float(playerAttributes['xA']), 'position': playerAttributes['position'], 'yellows': playerAttributes['yellow_card'], 'loc': 'h'}  # Adds player to playerDB if not already contained
                            ...
                            for playerID, playerAttributes in playerDict.items():
                                ...
                                playerDB[awayTeam][playerAttributes['player_id']][matchID] = {'mins': int(playerAttributes['time']), 'xG': float(playerAttributes['xG']), 'xA': float(playerAttributes['xA']), 'position': playerAttributes['position'], 'yellows': playerAttributes['yellow_card'], 'loc': 'a'}
